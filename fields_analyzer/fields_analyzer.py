...
def select_fields_to_analyze(json_data):
    ...
    for key in json_keys_array:
        for index_json_data in range(len(json_data)):
            if key == 'pid':
                ...
                pid_array.append(str(json_data[index_json_data][key]))
            else:
        ...
        if (type(array_to_analyze[0]) is int):  # **********YOU SHOULD JUSTIFY THIS*****************
            approved_fields_dict[
                key] = array_to_analyze  # if the diff_percentage is greater than 20% it is a categorical field.
    ...
    return np.array(
        array_to_return), pid_array  # here we have all the attributes from the entities in every array of the matrix
