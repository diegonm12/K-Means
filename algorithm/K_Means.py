...
class K_Means:
    ...
    def __init__(self, K, max_iterations=100):       ##this this the initialization
        ...
        self.clusters = [[] for _ in range(self.K)]     ## here we are creating an empty array for every cluster to be created.
    ...
    def predict(self,X):
        ...
        random_sample_index = np.random.choice(self.rows, self.K, replace = False)  #Here we use replace to avoid index duplicated. for example this line return: [0 2 1]
        self.centroids = [self.X[index] for index in random_sample_index]           #****************CHECK THIS***********************++
        ...
        for _ in range(self.max_iterations):
            ...
            self.centroids = self.get_centroids(self.clusters)      # we are calling this function to get the mean of the cluster
    ...
    def create_clusters (self, centroids):
        ...
        for index, point in enumerate(self.X):      # here we have the current index for the point
            centroid_index = self.closest_centroid(point, centroids)    # this function find the closest centroid for every point
            clusters[centroid_index].append(index)                      # here we are creating the clusters, using the index from the data set
        return clusters
    ...
    def closest_centroid(self, point, centroids):
        distances = [euclidean_dist(point, centroids_point) for centroids_point in centroids]       # we have here all the distances of every point to the centroid, we want the min
    ...
    def get_centroids(self, clusters):
        ...
        for cluster_index, cluster in enumerate(clusters):
            cluster_mean = np.mean(self.X[cluster], axis=0)     #remember that cluster are the indices, so that's why we are using "cluster" to get the mean
    ...
    def converge_centroids(self, previous_centroids, centroids):
        distances = [euclidean_dist(previous_centroids[element], centroids[element]) for element in range(self.K)]      # checking the distances between the old and current centroids
    ...
    def get_clusters_labeled(self,clusters):
        labeled = np.empty(self.rows)       # this is a empty array, labeled is the index of cluster the sample was assigned to
        for cluster_index, cluster in enumerate(clusters):
